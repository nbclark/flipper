...
public:
  ...
  BOOL Load(HWND hWnd, LPCWSTR lpRegPath, BOOL bUseNegId=FALSE) {
    RECT r;
    ...
    m_hWnd = hWnd;
    // save registry path
    m_szPath = new WCHAR [wcslen(lpRegPath)+1];
    m_bUseNegId = bUseNegId;
    if (m_szPath) { wcscpy(m_szPath, lpRegPath); }
...
protected:
  ...
  BOOL GetKey(HKEY hKeyRoot, LPCWSTR lpRegPath, LPCWSTR lpKey, LPVOID lpData, DWORD dwSize) {
  ...
  BOOL SetKey(HKEY hKeyRoot, LPCWSTR lpRegPath, LPCWSTR lpKey, LPVOID lpData, DWORD dwSize) {
    DWORD dwType = REG_BINARY;
    DWORD dwDisposition;
    HKEY  hKey;
  ...
  LPCWSTR i2s(UINT nNumber) {
    static WCHAR szn[34];
    return _itow(nNumber, szn, 16);
  }
  // variables
  HWND  m_hWnd;
  LPWSTR m_szPath;
